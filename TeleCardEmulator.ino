...
byte cardbytes[16] = { 0x99, 0x28, 0x1c, 0x02, 0xa1, 0xb3, 0x8d, 0xf4, 0x00, 0x22, 0x11, 0x77, 0x0f, 0xff, 0xff, 0xff };
byte cardbits[128];
...
void setup()
{
	...
	attachInterrupt(digitalPinToInterrupt(2), sendBit, FALLING);
	cardBytesToBits();
...
void loop()
{
	...
	if (line_RESET == 1){ 
		digitalWrite(pin_DATA, 0);	} //DATA line should always be low when RESET line is high
...
void sendBit(){
	...
	if (line_RESET == 0){ //DATA does not go out during RESET high
		addressBit++;
		digitalWrite(5, cardbits[addressBit]);	}
}
